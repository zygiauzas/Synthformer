...
import random
import random
from utils import getlistreactants, load_enamine_building_blocks
...
def compute_prod(bb,rxns,run):
    ...
    for l in range(10):
        ...
        for ri,rxn in enumerate(rxns):
            ...
            # Random sampling instead of iterating through all products
            sampled_products = random.sample(products, min(len(products), 1000)) if len(products) > 1000 else products
            for p in sampled_products:
                if count>10000:
                        break
                if  rxn.IsMoleculeReactant(p[0]):
                    # Random sampling of building blocks instead of iterating through all
                    sampled_bb = random.sample(list(enumerate(bb)), min(len(bb), 100)) if len(bb) > 100 else list(enumerate(bb))
                    for b2, bb2 in sampled_bb:
                        if count>10000:
                            break
