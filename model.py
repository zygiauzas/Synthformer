...
class Transformer(nn.Module):
    def __init__(self, source_vocab_size, target_vocab_size, source_max_seq_len, target_max_seq_len, embedding_dim, num_heads, num_layers, dropout=0.1):
        ...
        # Add fingerprint encoding layer for Z' = W * f_p(B_{i+1}) + b
        self.fingerprint_encoder = nn.Linear(1024, embedding_dim)
    ...
    def forward(self, p4, reactions, mflist,  buildingblock,buildingblockmf):
        ...
        # Get building block representations Z from decoder output
        bb_representations = output  # This is Z in the equation
        ...
        # Return building block representations for cosine similarity loss
        return buildingblockl, reaction, bb_representations
    def predict(self, p4, mflist, bbmf, end_token_id=None):
        """
        Autoregressive prediction following the inference protocol:
        1. Start with pharmacophore data and start token
        2. Predict building block B1, then reaction R0
        3. Use fingerprint of B1 to predict B2, then reaction R1
        4. Continue until end token is generated
        """
        # Encoder forward pass
        source_mask = torch.all(p4 == 0, dim=-1).unsqueeze(-2)
        ...
        logit=torch.multinomial(probbb[:,-1,:].squeeze(), 1)
        
        # Check for end token
        if end_token_id is not None and logit[0] == end_token_id:
            return logit, None, None  # End of sequence
            
        # Get fingerprint for selected building block
        if logit[0] < len(bbmf):
            # Use actual fingerprint from bbmf
            selected_fingerprint = torch.tensor(bbmf[logit[0]], dtype=torch.float32, device=p4.device)
            buildingblockmf = selected_fingerprint.unsqueeze(0).unsqueeze(0)
        else:
            # Fallback to embedding
            buildingblockmf = self.embedding(logit[0]).unsqueeze(0).unsqueeze(0)

        # Predict reaction using building block prediction and fingerprint
        merged_tensor = torch.cat((buildingblockl[:,-1,:], buildingblockmf[:,-1,:]), dim=-1)
        reaction = self.final_linear(merged_tensor)
        probr = self.softm(reaction)
        
        # Sample reaction
        logitr = torch.multinomial(probr.squeeze(), 1).item()
